...
export function getRequestUser(request) {
  ...
  const db = readDb();
  const instance = db.backendInstances.find((candidate) => candidate.tokenHash === tokenHash && !candidate.revokedAt);
  if (!instance) {
    return null;
  }

  mutateDb((mutableDb) => {
    const mutableInstance = mutableDb.backendInstances.find((candidate) => candidate.id === instance.id);
    if (mutableInstance) {
      mutableInstance.lastSeenAt = nowIso();
    }
  });

  return db.users.find((user) => user.id === instance.userId) || null;
}
