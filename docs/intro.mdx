...
No cloud is required, and an optional encrypted FrameOS Cloud backup can sync self-hosted backends and frames without exposing plaintext to the server.
...
Self-hosted backends can link to FrameOS Cloud for encrypted backups. Backend metadata, frame state, and frame assets are uploaded as encrypted envelopes, and the passphrase stays in the browser.

[Read the encrypted cloud backup guide](/guide/cloud-backups) or open [FrameOS Cloud backups](/backups).
