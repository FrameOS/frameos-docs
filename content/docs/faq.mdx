...
HDMI, no LCD, no Pimoroni HATs), no custom Nim apps (JavaScript instead), one
built-in font, and no headless Chromium screenshots or RTSP camera snapshots. The full picture is
on the [ESP32 page](/guide/esp32#what-it-cannot-do).
