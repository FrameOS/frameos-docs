...
4. Connect the board over USB and click **Flash from browser**. This uses Web Serial, so it works
   ...
   *USB JTAG/serial debug unit*, or a *USB Single Serial* UART bridge like the CH340 on the Seeed
   ...
   boards don't wire up). Or download the image and flash it by hand:
...
- **Serial console** (115200 baud) - always available, and quickest for tinkering. It answers on
  ...
  ```
  frameos> status                          # chip, panel, WiFi, memory, power policy
...
- **Over-the-air updates**: A/B partitions with automatic rollback - if a new build fails to come
  ...
  and verifies the image's SHA-256 against the manifest before installing it. A sleeping frame
...
charge in place of the status dot. Anything you deploy meanwhile is queued and lands on the next
