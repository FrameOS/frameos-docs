...
backups, activity log, security, and **Developer** for [API tokens and the MCP server](#developer)).
...
2. Name the frame, pick its display (or choose to pick it later from the frame's setup hotspot),
   and enter your WiFi network. Optionally set a root password and tick the **SSH keys** that
   should be able to log in as root - add one right there or under **Settings → SSH keys**; the
...
- **Scenes.** Install from the [scene store](/guide/scene-store), or open a scene in the built-in
  ...
  own edit of it, and which version is on the frame.
...
- **Sleep forecast.** A frame that deep-sleeps says so before it halts: the sidebar and the
  ...
  and battery frames show their charge in place of the status dot.
...
- **Personal API tokens** (`fc_api_…`, or a read-only `fc_apiro_…`) stand in for your account on
  ...
  sign-in in the browser (revoking frames, approving device links). Minting one needs a recent
  sign-in too; tokens are shown once and stored hashed.
- **A hosted MCP server** at `https://cloud.frameos.net/api/mcp` - the same API with a vocabulary a
  [Model Context Protocol](https://modelcontextprotocol.io/) client can read: 71 tools across
  ...
  from JSON, zip or URL, fork, publish, gallery images, yank, delete), the store, the scene AI, and
  a server-side **render** that runs the real FrameOS runtime on any scene JSON and hands the
  image back, plus the **lint** that publishing applies. Nothing local to run: it speaks Streamable
