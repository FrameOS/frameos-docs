...
password, or **Continue with Google**. Verify your email, sign in, and you land on **Frames**.
...
2. Name the frame, pick its display (or choose to pick it later from the frame's setup hotspot),
   and enter your WiFi network. Optionally set a root password.
...
hostname from the frame name (`kitchen-frame.local`), and enrolls. The panel shows the FrameOS
...
immediately; every later one appears as **pending** and waits for you to click **Confirm frame**, so
a lost SD card can't quietly add frames to your account. A dropdown on the builder lets you expire
the image after a while if you prefer.
...
Each frame has an overview page at `/frames/<id>` and tools under it: **Settings**, **Preview**,
**Schedule**, **Logs**, **Metrics**, **Assets** and **Activity**.
