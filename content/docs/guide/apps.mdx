...
| Custom Nim apps | Compiled scenes only | ❌ | ❌ |
| Nim code nodes | Compiled scenes only | ❌ - use JS code nodes | ❌ |
...
own fonts and drawn as outlines, so labels come out sharp at any size. FrameOS implements a subset
of SVG rather than the whole spec: `<defs>` and external references are ignored, and on a
microcontroller everything renders in the one built-in typeface.
...
fallback for characters your font doesn't cover.
