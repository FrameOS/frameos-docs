...
- **Adopt frame** - take over a Pi that is already running FrameOS on its own (booted from a
  ...
  at this backend. Logs start flowing after the frame's next restart or deploy. ESP32 frames
...
resolution, network address, time zone, who manages it and "Waiting for the backend to deploy a
scene." Once it has scenes, that same screen lists them - handy when a panel looks blank and you
want to know whether the frame is fine.

![Adding a frame](/images/app/add-frame.gif)

## Install some scenes

Install a few scenes onto the frame, from the built-in repositories or the
[scene store](/guide/scene-store). Good first picks:
